...
- 自动记住上次选择的输出目录。
- 支持多条自动整理规则，后台监控新增电影、电视剧与字幕并自动创建硬链接。
- 新文件保持大小稳定达到设定时间后才处理，避免下载未完成时提前入库。
- 无法识别剧集编号或发现目标冲突时停止该文件，并在网页显示运行记录。
- Windows 安装版与 Linux 客户端支持开机后后台自启动；Docker 版跟随容器启动。
...
- 自动更新失败时停止循环重试，并在网页显示具体错误、安装日志位置和“重试更新”按钮。
- 未启用自动整理时，关闭所有 MediaLinker 网页后打包版后台服务会自动退出；启用后则继续在后台监控。

## 自动整理服务

在网页右上角打开“设置 → 自动整理服务”，即可添加一条或多条监控规则。每条规则对应一部电影或一部电视剧，需要配置：
...
保存并启用后，MediaLinker 会在后台定期扫描。文件大小持续不变达到设置的等待时间后才会创建硬链接。电视剧文件名需要包含 `S01E01`、`E01`、`EP01` 或“第1集”等可识别编号；无法可靠识别的文件不会被猜测命名，而会保留在运行记录中等待处理。

自动整理模式只创建硬链接，不会移动或重命名下载目录中的原始文件。输出目录不能放在监控目录内部，来源和输出仍必须位于同一个文件系统。

### 后台与开机自启动

- Windows 安装版/绿色版：启用“开机后自动在后台启动”后写入当前用户启动项，不需要管理员权限，也不会显示命令行窗口。
- 普通 Linux 便携版：创建当前用户的桌面自动启动项。
- Flatpak：通过桌面系统的后台运行门户申请开机自启动，首次启用时桌面可能显示授权提示。
- NAS / Docker：使用 `restart: unless-stopped`，由 Docker 自动随系统启动。
- 点击正常的 MediaLinker 图标时，如果后台服务已经运行，会直接打开现有管理页面，不会重复启动第二个服务。
